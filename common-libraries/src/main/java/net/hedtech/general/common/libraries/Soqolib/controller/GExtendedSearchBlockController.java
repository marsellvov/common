...
public class GExtendedSearchBlockController extends AbstractSupportCodeObject {
	...
	private net.hedtech.general.common.libraries.Goqolib.controller.GExtendedSearchBlockController getGoqolibGExtendedSearchBlockController() {
		return (net.hedtech.general.common.libraries.Goqolib.controller.GExtendedSearchBlockController) this.getContainer().getTaskPart("GOQOLIB").getSupportCodeManager().getPackage("G$_EXTENDED_SEARCH_BLOCK");
	}	
...
END;
		...
		*/
		...
		{
			net.hedtech.general.common.libraries.Goqolib.model.GExtendedSearchBlock gExtendedSearchBlockElement = this.getFormModel().getGExtendedSearchBlock(); 
				if(gExtendedSearchBlockElement.getSrSpbpersBirthDate().isNull())
		return;
...
IF :SR_SPBPERS_SEX NOT IN ('M','F','N','m','f','n') THEN
   MESSAGE( G$_NLS.Get('SOQOLIB-0428', 'FORM','*ERROR* Invalid code. Valid values are (M)ale, (F)emale, (N)ot available.') ); 
...
END IF;
		...
		*/
		...
		public void srSpbpersSex_validate()
		{
				...
				if ( !in(getFormModel().getGExtendedSearchBlock().getSrSpbpersSex(), "M", "F", "N", "m", "f", "n").getValue() )
				{
					errorMessage(GNls.Fget(toStr("SOQOLIB-0428"), toStr("FORM"), toStr("*ERROR* Invalid code. Valid values are (M)ale, (F)emale, (N)ot available.")));
