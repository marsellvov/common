...
import net.hedtech.general.common.libraries.Goqolib.services.Exceptions.*;
...
END;
		...
		*/
		...
		/* Original PL/SQL code code for TRIGGER G$_FORM_CLASS.
		 DECLARE
--
		...
		*/
		...
		public void gFormClass_GVerifyAccess()
		{
			...
			try {
				...
				GSecurity.gVerifyPassword1Prd(_object, version, refPassword, refRoleName);
				...
				if (!password.equals("INSECURED")) {
					...
					if (formFileName.notEquals(formModName)) {
						errMsg = GNls.Fget(toStr("GOQOLIB-0030"), toStr("FORM"), toStr("*ERROR* User attempted to run %01% but the internal name (%02%) does not match."), formFileName, formModName);
						//
					...
					getContainer().getGoqrpls().getGTracePkg().doExecSql(toStr("SET ROLE ").append(holdCmd));
					//
				...
				errorMessage(GNls.Fget(toStr("GOQOLIB-0031"), toStr("FORM"), toStr("*ERROR* No parameters were passed.")));
				throw new ApplicationException();
			} catch (NoInst e) {
				errorMessage(GNls.Fget(toStr("GOQOLIB-0032"), toStr("FORM"), toStr("*ERROR* No records found on GUBIPRF.")));
				throw new ApplicationException();
			} catch (NoAccess e) {
				errorMessage(GNls.Fget(toStr("GOQOLIB-0033"), toStr("FORM"), toStr("*ERROR* User %01% not authorized to access %02%"), getUser(), _object));
				throw new ApplicationException();
			} catch (NoPassword e) {
				errorMessage(GNls.Fget(toStr("GOQOLIB-0034"), toStr("FORM"), toStr("*ERROR* No password found on GUBROLE.")));
				throw new ApplicationException();
			} catch (InvalidVersion e) {
				errorMessage(GNls.Fget(toStr("GOQOLIB-0035"), toStr("FORM"), toStr("*ERROR* Invalid version of %01%."), _object));
				throw new ApplicationException();
			} catch (InvalidAccess e) {
				errorMessage(GNls.Fget(toStr("GOQOLIB-0036"), toStr("FORM"), toStr("*ERROR* Invalid password tried.")));
				throw new ApplicationException();
		...
		public void gFormClass_GRevokeAccess()
		{
...

				{
					getContainer().getGoqrpls().getGVpdi().revokeAccess();
					 ...
					 {
						 getContainer().getGoqrpls().getGTracePkg().doExecSql(toStr("SET ROLE NONE"));
					 }
					 //
					 if ( getNameIn("GLOBAL.SKIP_FORM_CNT").equals("Y") || getNameIn("GLOBAL.SKIP_DECR_CNT").equals("Y") || getCurrentTaskName().equals("GUAMENU") ||
							 getCurrentTaskName().equals("GUAGMNU") || (getTriggerBlock()) != null)
					 {
						 copy("N","GLOBAL.SKIP_DECR_CNT");
						 return ;
					 }
					...
					//   COPY(CURR_FORMS,'GLOBAL.CURR_OPEN_FORMS');
					//   
					NNumber tempNo = toNumber(toNumber(getNameIn("GLOBAL.CURR_NO_FORMS")).subtract(1));
...
//
		...
		*/
		 ...
		 *</p>
		*/
		public void gFormClass_WhenCustomJavascriptEvent(String eventName, String eventValue)
		{
				//This action is used in the handling of the FORM_EVENT action, that replaces the original WHEN-CUSTOM-JAVASCRIPT-EVENT
...
END;
		...
		*/
		...
		public void gFormClass_WhenNewBlockInstance()
		{
			...
			if (getContainer().getGoqrpls().getGNavigationFrame().messageWasDisplayed.equals("N")) {
			...
			// procedure will not perform any Workflow function.
			if (isNull(getGlobal("WF_ENABLED_IND"), "N").equals("Y")) {
				getContainer().getGoqwflw().getGWfDriver().wfExecute(toStr("WF_CONTROL_KEY_BLOCK"));
			}
...
END;
		...
		*/
		...
		public void gFormClass_WhenTimerExpired(NString timerName)
		{
				...
				//F2J_WRAPPED_CODE : The code of this Trigger was commented out so that you can migrate, compile and check your code in an iterative way.
//
//				getContainer().getGoqrpls().gTimerExp();
//				getContainer().getGoqrpls().gCheckFailure();
//				// 
//				//  4000-32 Add the following conditional call to the G$_WF_DRIVER.WF_EXECUTE
//				//          procedure to check for a valid task or activity sent via Workflow
...
END;
		...
		*/
		...
		public void gFormClass_InvalidOptionMsg()
		{
				...
				errorMessage(GNls.Fget(toStr("GOQOLIB-0038"), toStr("FORM"), toStr("*ERROR* Selected Option not available from this block.")));
				throw new ApplicationException();
...
END;
		...
		*/
		...
		{
				getContainer().getGoqrpls().getGIdnameSearch().idSearch(toStr("ID"), toStr(""), toStr(""));
				getContainer().getGoqrpls().gCheckFailure();
...
//						}
//						else {
//							getContainer().getGoqgmnu().getGMultiRecSelection().multiRecordChoice(toStr("DELETE"), lvBlockName.append(".GURMENU_OBJ_NAME"), lvBlockName.append(".GURMENU_COUNT"));
//						}
//					}
//				}
//
