...
public class Viewwmnu {
		public static void pStart(NString returnCode) {
			
			IDataCommand cmd = DbManager.getDataAccessFactory().createDataCommand("VIEWWMNU.P_START", DbManager.getDataBaseFactory());
		...
		public static void pWebMenuDisplay(NString module,NString tableType,NString wmnuTableName,NString wmrlTableName,NString wmnuTableOwner,NString wmrlTableOwner,NString instTableOwner) {
			...
			IDataCommand cmd = DbManager.getDataAccessFactory().createDataCommand("VIEWWMNU.P_WEB_MENU_DISPLAY", DbManager.getDataBaseFactory());
