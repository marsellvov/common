...
--  MESSAGE(G$_NLS.Get('GOQCLIB-0015','FORM','*ERROR* NEXT is not valid for this form.'));
...
G$_CHECK_FAILURE;
--
:KEY_BLOCK.PIDM := :SPRIDEN_CURRENT.SPRIDEN_PIDM;
		*/
		/*
		 *<p>
		 *<b>Migration Comments</b>
		...
		*/
		...
		public void spridenCurrent_Save()
		{
				...
				getTask().getGoqrpls().gCheckFailure();
				// 
				getFormModel().getKeyBlock().setPidm(spridenCurrentElement.getSpridenPidm());
			}
		}
		...
		*/
		...
		public void spridenCurrent_RequestNameSearch()
		{
			...
			getGoqclibSpridenCurrentController().spridenCurrent_RequestNameSearch();
		}
...
--
--  IF :SPRIDEN_CURRENT.SPRIDEN_ID = 'NEXT' THEN
--    MESSAGE( G$_NLS.Get('GOQCLIB-0035', 'FORM','NEXT is not valid for this form.') );
--    RAISE FORM_TRIGGER_FAILURE;
--  END IF;
--
  IF (:SPRIDEN_CURRENT.SPRIDEN_PIDM IS NULL AND
      :KEY_BLOCK.ID <> :SPRIDEN_CURRENT.SPRIDEN_ID) THEN
    MESSAGE( G$_NLS.Get('SPAIDEN-0011', 'FORM','*ERROR* You may not change current ID on an add; change ID in key block.') );
    RAISE FORM_TRIGGER_FAILURE;
  END IF;
--
...
--
  ...
  IF PTI_CURSOR1%FOUND THEN
    CLOSE PTI_CURSOR1;
    MESSAGE(G$_NLS.Get('SPAIDEN-0012','FORM','*ERROR* ID number entered is current ID number for another person or non-person.'));
    RAISE FORM_TRIGGER_FAILURE;
  END IF;
  CLOSE PTI_CURSOR1;
--
...
--
  ...
  IF PTI_CURSOR2%FOUND THEN
    CLOSE PTI_CURSOR2;
    MESSAGE(G$_NLS.Get('SPAIDEN-0013','FORM','*ERROR* ID number entered is previous ID number for another person or non-person.'));
    RAISE FORM_TRIGGER_FAILURE;
  END IF;
  CLOSE PTI_CURSOR2;
--
...
-- -- --
  IF :GLOBAL.SSN_SEARCH_ENABLED_IND = 'Y' THEN
    ...
    IF SSN_CURSOR%FOUND THEN
      CLOSE SSN_CURSOR;
      MESSAGE(G$_NLS.Get('SPAIDEN-0014','FORM','*ERROR* ID number entered is an SSN/SIN/TIN.'));
      RAISE FORM_TRIGGER_FAILURE;
    END IF;
    CLOSE SSN_CURSOR;
  END IF;
	...
	" WHERE SPBPERS_SSN = :SPRIDEN_CURRENT_SPRIDEN_ID AND SPRIDEN_CHANGE_IND IS NULL AND SPRIDEN_PIDM = SPBPERS_PIDM ";
					...
					try {
						try
						{
							...
							{
								errorMessage(GNls.Fget(toStr("SPAIDEN-0011"), toStr("FORM"), toStr("*ERROR* You may not change current ID on an add; change ID in key block.")));
							...
							if ( ptiCursor1.found() )
							{
								errorMessage(GNls.Fget(toStr("SPAIDEN-0012"), toStr("FORM"), toStr("*ERROR* ID number entered is current ID number for another person or non-person.")));
								throw new ApplicationException();
							}
							// 
							//  This trigger step checks if the changed id number is the previous id of
							...
							if ( ptiCursor2.found() )
							{
								errorMessage(GNls.Fget(toStr("SPAIDEN-0013"), toStr("FORM"), toStr("*ERROR* ID number entered is previous ID number for another person or non-person.")));
								throw new ApplicationException();
							}
							// 
							//  -- -- This trigger step checks if the changed id number is an SSN/SIN/TIN of
							...
							if ( getGlobal("SSN_SEARCH_ENABLED_IND").equals("Y") )
							{
								...
								if ( ssnCursor.found() )
								{
									errorMessage(GNls.Fget(toStr("SPAIDEN-0014"), toStr("FORM"), toStr("*ERROR* ID number entered is an SSN/SIN/TIN.")));
									throw new ApplicationException();
								}
							}
							//  -- --
