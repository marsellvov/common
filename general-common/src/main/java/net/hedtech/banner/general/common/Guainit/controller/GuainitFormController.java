package net.hedtech.banner.general.common.Guainit.controller;
...
import net.hedtech.banner.general.common.Guainit.services.*;
...
public class GuainitFormController extends DefaultFormController {
	...
	public GuainitFormController(ITask task) {
		super(task);
	...
	public GuainitTask getTask() {
		return (GuainitTask) super.getTask();
	}
...
//		setGlobal("NLS_DATE_FORMAT", isNull(getEnvVar("NLS_DATE_FORMAT"), getGlobal("NLS_DATE_FORMAT")));
	...
	public void Guainit_PostLogon() {
		...
		int rowCount = 0;
		{
			...
			try {
				...
				GSecurity.gAuditBannerLogon(/* pSource=> */toStr("INB"), /*pComment=*/toStr("Logon to Banner Forms"));
			} catch (Exception e) {
	 ...
	 */
	@TaskStartedPre
	public void Guainit_TaskStartedPre(EventObject args) {
		...
		this.Guainit_PostLogon();
		{
			...
			setGlobal("FORMNAME_DISPLAY_IND", toStr("N"));
	 ...
	 */
	@TaskStarted
	public void Guainit_TaskStarted(EventObject args) {
		...
		int rowCount = 0;
		{
			...
			NString lFormsUsernameCasesensi = NString.getNull();
	...
	/*
	 * Original PL/SQL code code for TRIGGER GUAINIT.KEY-OTHERS BEGIN
	 * EXECUTE_TRIGGER('LEAVE_INIT'); END;
	 */
	/*
	 * <p><b>Migration Comments</b> Generated from trigger: GUAINIT.KEY-OTHERS
	 * 
	 * 
	 * </p>
	 */
	@ActionTrigger(action = "KEY-OTHERS")
	public void Guainit_KeyOthers() {
