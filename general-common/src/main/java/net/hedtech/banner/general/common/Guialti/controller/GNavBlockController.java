...
END;
  ...
  IF :SYSTEM.FORM_STATUS = 'CHANGED' THEN
    MESSAGE( G$_NLS.Get('GUIALTI-0001', 'FORM','*ERROR* There are pending form changes; Commit or Rollback.') );
...
END;
		...
		*/
		...
		public void goItem_keyNexItem()
		{
				...
				{
					errorMessage(GNls.Fget(toStr("GUIALTI-0001"), toStr("FORM"), toStr("*ERROR* There are pending form changes; Commit or Rollback.")));
...
--
  IF :SYSTEM.FORM_STATUS = 'CHANGED' THEN
    MESSAGE( G$_NLS.Get('GUIALTI-0002', 'FORM','*ERROR* There are pending form changes; Commit or Rollback.') );
    RAISE FORM_TRIGGER_FAILURE;
  END IF;
--
...
END;
		...
		*/
		...
		public void goItem_KeyEnter()
		{
				...
				if ( getTaskStatus().equals("CHANGED") )
				{
					errorMessage(GNls.Fget(toStr("GUIALTI-0002"), toStr("FORM"), toStr("*ERROR* There are pending form changes; Commit or Rollback.")));
					throw new ApplicationException();
				}
				if ( inStr(getFormModel().getGNavBlock().getGoItem(), toStr("%")).greaterOrEquals(1) )
				{
		...
		*/
		...
		/* Original PL/SQL code code for TRIGGER GO_ITEM_BTN.WHEN-BUTTON-PRESSED
		 BEGIN
  IF :SYSTEM.FORM_STATUS = 'CHANGED' THEN
    MESSAGE( G$_NLS.Get('GUIALTI-0003', 'FORM','*ERROR* There are pending form changes; Commit or Rollback.') );
    RAISE FORM_TRIGGER_FAILURE;
  END IF;
--
...
END;
		...
		*/
		...
		public void goItemBtn_buttonClick()
		{
				...
				if ( getTaskStatus().equals("CHANGED") )
				{
					errorMessage(GNls.Fget(toStr("GUIALTI-0003"), toStr("FORM"), toStr("*ERROR* There are pending form changes; Commit or Rollback.")));
					throw new ApplicationException();
				}
