package net.hedtech.banner.general.common.BannerMain.services;
...
import morphis.core.utils.misc.Func;
import morphis.core.utils.misc.Predicate;
...
import morphis.workspace.forms.main.services.MainServices;
...
public class BannerMainServices extends MainServices {
		
	public BannerMainServices(ISupportCodeContainer container) {
		super(container);
	}
	
	public BannerMainTask  getTask() {
		return (BannerMainTask )super.getContainer();
	}

	
	public net.hedtech.banner.general.common.BannerMain.model.BannerMainModel getFormModel() {
	...
	public void startApplication() {
		...
		if (!getGlobal("MEP_INIT_RETURN").equals("Y")){
		
			//JOG 06/01/2015 - Banner Guainit replacement
			executeAction("ON-LOGON");
			executeAction("POST-LOGON");
			executeAction("START");
		}
		if (getGlobal("MEP_INIT_RETURN").equals("Y") || !getGlobal("MEP_INIT").equals("Y")){
			super.startApplication();
		}
	}
	
	@Override
	public void callForm(String mode, String taskName, Hashtable<?, ?> parameterList, boolean sync) {

		//JOG 06/01/2015 - Banner Guainit replacement
		
		//Check the open tasks
		List<ITask> allOpenTasks = TaskServices.getTasks(new Predicate<ITask>() {		
			@Override
			public Boolean execute(ITask t) {
				return !t.isWorkspace();
			}
		});

		
		if ( allOpenTasks.isEmpty() ) {		
			//launch the form using the Guainit start launch (as in leave_init)
			this.getTask().getServices().launchFormLogic(toStr(taskName), sync);			
		}
		else {
			
			//check if the form is already open
			{		
				//launch the form using the XE Navigate mechanism on top of the 'current' form
				ITask callerTask = null;
				
				ITask currentTask = TaskServices.getCurrentTask();
				if ( !currentTask.isWorkspace() )
					callerTask = currentTask;
				else {
					//TODO: this supports only one 'chain' of open forms. If there are several, (eg different tabs) this mechanism will not work properly
					//The choice must come from the interface, corresponding to the selected tab
					
					//get the deepest form
					callerTask = CollectionUtils.maxBy(allOpenTasks, new Func<ITask, Integer>() {
						@Override
						public Integer execute(ITask e) {
							int i = 0;
				...
				if ( callerTask != null ) {
					...
					TaskServices.setCurrentTask(callerTask);
					//TODO: use a version of executeTask("FORM_EVENT", { "NAVIGATE", "{page:%s,values:}" } )
					//Simulating a form_event
					((DefaultFormController)callerTask.getFormController()).formEvent("NAVIGATE", String.format("{page:%s,values:, parameters: {RELATED_FORM_CALL:\"Y\"}}",taskName) );
				}
			}
		}
...
/* </p>
		* @param pInvokeObject
		*/
		public void launchFormLogic(NString pInvokeObject, boolean sync)
		{
			int rowCount = 0;
	...
	" WHERE GUBOBJS_NAME = :P_LV_NAV_VALUE ";
				...
				NString modeIn = sync ? toStr("HIDE") : toStr("");
				getTask().getGoqrpls().gSecuredFormCallPl(getGlobal("CURRENT_USER"), lvNavValue, plId, modeIn);
			}
...
/* </p>
		*/
		...
		public void guagMenu_doFormCall(MessagesAdapter messagesElement, NString formNameIn) 
		{
			// 
