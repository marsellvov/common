...
END;
		...
		*/
		...
		public void idnameExtsearchList_listChange()
		{
				...
				getTask().getGoqrpls().getGIdnameSearch().copySelectedId();
			}
...
END;
		...
		*/
		...
		{
			GExtendedSearchBlock gExtendedSearchBlockElement = this.getFormModel().getGExtendedSearchBlock();
...
IF :SR_SPBPERS_SEX NOT IN ('M','F','N','m','f','n') THEN
   MESSAGE( G$_NLS.Get('GOAEACC-0012', 'FORM','*ERROR* Invalid code. Valid values are (M)ale, (F)emale, (N)ot available.') ); 
...
END IF;
		...
		*/
		...
		public void srSpbpersSex_validate()
		{
				...
				getTask().getGoqrpls().getGIdnameSearch().whenNewItem();
				if ( !in(getFormModel().getGExtendedSearchBlock().getSrSpbpersSex(), "M", "F", "N", "m", "f", "n").getValue() )
				{
					errorMessage(GNls.Fget(toStr("GOAEACC-0012"), toStr("FORM"), toStr("*ERROR* Invalid code. Valid values are (M)ale, (F)emale, (N)ot available.")));
