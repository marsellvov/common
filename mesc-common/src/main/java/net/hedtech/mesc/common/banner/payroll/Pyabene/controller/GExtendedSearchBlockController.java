...
IF :SR_SPBPERS_SEX NOT IN ('M','F','N','m','f','n') THEN
   MESSAGE( G$_NLS.Get('PYABENE-0062', 'FORM','*ERROR* Invalid code. Valid values are (M)ale, (F)emale, (N)ot available.') ); 
...
END IF;
		...
		*/
		...
		public void srSpbpersSex_validate()
		{
				...
				{
					errorMessage(GNls.Fget(toStr("PYABENE-0062"), toStr("FORM"), toStr("*ERROR* Invalid code. Valid values are (M)ale, (F)emale, (N)ot available.")));
